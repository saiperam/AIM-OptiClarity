  ...
  const sendFile = async () => {
    if (selectedFile) {
      ...
      try {
        ...
        } else if (selectedOption === "Fundus") {
          // For Fundus, use localhost:8004
          res = await axios.post("http://localhost:8004/predict", formData);
        ...
        } else if (selectedOption === "Keratoconus") {
          // For Keratoconus, use localhost:8003
          res = await axios.post("http://localhost:8003/predict", formData);
        }
        if (res.status === 200) {
          setData(res.data);
          setConfidence(
            res.data.confidence !== undefined
  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-4">
        ...
        <p className="text-gray-600 sm:text-lg text-base text-center mt-6">
          Get fast, reliable insights from your OCT, Fundus and various other
          scans with our AI-powered analysis.
        </p>
        <Workflow />
        ...
        {selectedOption && (
          <div className="mt-6 border-2 border-dashed border-gray-300 rounded-lg p-8 text-center hover:border-blue-500 transition">
            ...
            <h3 className="text-xl font-semibold mb-2">
              Upload {selectedOption} Scan
            </h3>
            <p className="text-gray-600 mb-4">
              Drag and drop your {selectedOption.toLowerCase()} scan or click to
              browse
            </p>
            <button
      ...
      {showPopup && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
          <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-lg md:w-96 w-80 relative">
            <button
              onClick={clearData}
              className="absolute top-2 right-2 text-gray-700 p-2"
            >
              <X size={20} />
            </button>
            {isLoading ? (
              ...
              <div className="text-center">
                <img
                  src={preview}
                  alt="Uploaded scan"
                  className="md:w-80 md:h-80 w-60 h-60 object-cover rounded-lg mx-auto mb-4"
                />
                <div className="text-lg font-semibold">{data?.prediction}</div>
                {selectedOption !== "Fundus" &&
